...
type TupleHasDuplicate<T extends readonly unknown[]> = T extends [infer F, ...infer R]
    ? F extends R[number]
        ? true
        : TupleHasDuplicate<R>
    : false;

type NoDuplicates<T extends readonly unknown[]> = TupleHasDuplicate<T> extends true ? never : T;

const auth = <T extends readonly (UserRoleEnum | 'ANY')[]>(...roles: NoDuplicates<T> extends never ? never : T) => {
        ...
        try {
            ...
            if (roles.includes('ANY')) {
                ...
                if (roles.length && !roles.includes(verifyUserToken.role)) {
                    throw new AppError(httpStatus.FORBIDDEN, 'You are not authorized to access this api!');
                }
                next();
            }
        } catch (error) {
